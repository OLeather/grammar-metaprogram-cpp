...
using Char    = Or<Regex<R"(\\[nrt'"\[\]\\])">, Regex<R"(\\[0-2][0-7][0-7])">, Regex<R"(\\[0-7][0-7]?)">, Sequence<Not<Regex<R"(\\)">>, Regex<R"(.)">>>;
using Range   = Or<Sequence<Char, Regex<R"(-)">, Char>, Char>;
using Class   = Sequence<Regex<R"(\[)">, Repeated<Sequence<Not<Regex<R"(\])">>, Range>>, Regex<R"(\])">, Spacing>;
using Literal = Or<
  Sequence<Regex<R"(')">, Repeated<Sequence<Not<Regex<R"(')">>, Char>>, Regex<R"(')">, Spacing>,
  Sequence<Regex<R"(")">, Repeated<Sequence<Not<Regex<R"(")">>, Char>>, Regex<R"(")">, Spacing>
>;
...
struct Expression;
using Primary = Or<Sequence<Identifier, Not<LEFTARROW>>, Sequence<OPEN, Eval<Expression>, CLOSE>, Literal, Class, DOT>;
using Suffix  = Or<Sequence<Primary, Or<QUESTION, STAR, PLUS>>, Primary>;
using Prefix  = Or<Sequence<Or<AND, NOT>, Suffix>,Suffix>;

using SequenceRule = Repeated<Prefix>;
struct Expression  : Sequence<SequenceRule, Repeated<Sequence<SLASH, SequenceRule>>>{};
using Definition   = Sequence<Identifier, LEFTARROW, Expression>;
using Grammar      = Sequence<Spacing, Repeated<Definition>, EndOfFile>;
// clang-format on
